...
dailyPhpMyAdminForImport=$SCRIPTS/sql/event_style_status_plusimgcpy_live.sql

dbOutRaw=$PRODSRV/tmp/limbo/spool_eventcolorstylesplus.csv
dbOutClean=$PRODSRV/data/csv/events_style_statusplus.csv
...
##source ~/.bash_profile && sudo port load mysql55-server && sudo mysql5 --socket=/Applications/MAMP/tmp/mysql/mysql.sock --password=root --database=imageMetaData < $dbOutClean ;
sleep 3
