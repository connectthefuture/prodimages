...
convert $f -format jpg -profile $srgb_webrdy -crop \
  `convert "$f" -virtual-pixel edge -blur 0x15 -fuzz 1% -trim \
   -format '%wx%h%O' \
   info:` -background white +repage -gravity center -resize '700x840' -profile $srgb_webrdy -background white +repage -extent '725x870' -colorspace sRGB -unsharp 15 -quality 85 "$f"_new.jpg
#fi ;
