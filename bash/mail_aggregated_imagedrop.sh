...
msql="select distinct count(colorstyle) as style_ct, brand, product_type, (CASE WHEN ( production_complete_dt = current_date() ) or (image_ready_dt is null and production_complete_dt is null and copy_ready_dt is not null) THEN 1 ELSE 0 END) complete_today, (CASE WHEN production_complete_dt < current_date() THEN 1 ELSE 0 END) reshoot_reload from product_snapshot_vendor where colorstyle in ${mainstyles} group by brand, product_type order by 1 desc, 2 asc;"
asql="select distinct count(colorstyle) as style_ct, brand, product_type, (CASE WHEN ( production_complete_dt = current_date() ) or (image_ready_dt is null and production_complete_dt is null and copy_ready_dt is not null) THEN 1 ELSE 0 END) complete_today, (CASE WHEN production_complete_dt < current_date() THEN 1 ELSE 0 END) reshoot_reload from product_snapshot_vendor where colorstyle in ${altstyles} group by brand, product_type order by 1 desc, 2 asc;"
echo "$msql"
echo "$asql"
