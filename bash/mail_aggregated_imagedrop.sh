...
echo "$msql"
echo "$asql"
main_results=`mysql --host=127.0.0.1 --port=3301 --column-names=True --table --user=root --password=mysql -e $(echo -e "$msql") -D www_django`
alt_results=`mysql --host=127.0.0.1 --port=3301 --column-names=True --table --user=root --password=mysql -e $(echo -e "$asql") -D www_django`
