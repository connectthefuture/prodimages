...
$SCRIPTS/outputStringsIOargs.sh $outFileStringPairs

## RUN SQL Query spooling Meta Tags to CSV
##newMetaTagSQL=$SQLSCRIPTS/newMetaTagSQL.sql
newMetaTagSQL=$SQLSCRIPTS/allFullSampleQNewerExifSQL.sql
##newMetaTagSQL=$SQLSCRIPTS/onlySampDTSinceAPR2012_SQL.sql
...
exifHeader=$PRODSRV/lib/exifHeaders.csv
...
cat $outFileStringPairs | awk '{ print $2 }' | grep -v _MAC | sort -t, -bdri -k 3.2b,3.2br -k 3.5b,3.5br | uniq | awk -F, '{ print "exiftool -m -f -P -d %Y-%m-%d -overwrite_original_in_place -fast2 -csv='"$exifReadHeaded"' "$1 }' | /bin/bash
exit;
