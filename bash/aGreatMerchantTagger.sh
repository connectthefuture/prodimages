...
merchantMetaTagSQL=$SQLSCRIPTS/daily_MerchantInfoTags.sql
###         <----Make dbOutRaw
$DSSPRDLOGIN @$merchantMetaTagSQL;


###<----Fix Sql Output Format
cat $dbOutRaw | sed s/'  '//g | sed s/'--'//g | sed s/' ,'/','/g | sed s/'IPTC:Samp,'/'IPTC:SampleStatusDate,'/1 | sort -k1 -t"," -du > $dbOutClean
...
exifHeaders=$PRODSRV/lib/exifHeadersMerchant.csv
exifReadHeaded=$LIMBO/exifReadWithHeadReshoot.csv
