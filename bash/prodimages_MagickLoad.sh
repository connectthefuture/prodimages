...
tmp_processing=$dropBase/.tmp_processing
#userDropFolders="$1"
cd $tmp_processing
## make and go to Base folder for portability using unix time to protect from long processes running into each other

mkdir magickBase"$tmpFolderTime"
magickBaseR=magickBase"$tmpFolderTime"/
cd $magickBaseR
magickBase=`pwd`
...
/usr/bin/rsync --archive --compress --update --verbose $uploadComplete/ $completeArch
#/mnt/Post_Complete/Complete_to_Load/ /mnt/Post_Complete/Complete_Archive
