...
fileAttach=$PRODSRV/mail/outbox/compiledPhotoReport
fileHTML=$PRODSRV/mail/outbox/compiledPhotoReport.html
cat $filesAll | grep -v "^-" | awk -v ORS=";" '{ print $0 }' | sed 's/2\ 2\ 1/\-----Photo_Date-----/1' | sed 's/2\ 2\ 1/\-----Product_Type-----/1' | sed 's/\ \&\ /_\&_/g' | sed 's/\ shirts/_shirts/g' | sed 's/\ goods/_goods/g' | awk -v RS=";" '{ print $1,$2 }' > ''$fileAttach'_styles.csv'


cat $filesAllShots | grep -v "^-" | awk -v ORS=";" '{ print $0 }' | sed 's/2\ 2\ 1/\-----Photo_Date-----/1' | sed 's/2\ 2\ 1/\-----Product_Type-----/1' | awk -v RS=";" '{ print $1,$2,$3,$4,$5,$6 }' > ''$fileAttach'_shots.csv'

cat ''$fileAttach'_shots.csv' | awk '{ print $NF }' > ''$fileAttach'_shotcount.csv'
rm ''$fileAttach'_shots.csv'

paste ''$fileAttach'_styles.csv' ''$fileAttach'_shotcount.csv' > ''$fileAttach'_combined.csv'

#stylesTotal=`cat ''$fileAttach'_styles.csv' | head -1`
#shotTotal=`cat ''$fileAttach'_shots.csv' | head -1`
myMailAddress="jbragato@bluefly.com john.bragato@gmail.com"
allMailAddress='<jbragato@bluefly.com> <conrad.sanderson@bluefly.com>'
...
mail -s "stylesTotal shotTotal" "$allMailAddress" < ''$fileAttach'_combined.csv'
