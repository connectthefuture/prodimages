...
//		if (isset($_POST['qtable'])) $qtable = $_POST['qtable'];
        if (isset($_POST['jointable'])) $jointable = $_POST['jointable'];
			else
			{
			$jointable = 'data_imagepaths.product_snapshot';
			}
...
//			echo $query;
	...
	////////////////// Regular Search	/////////////////	
		else if(isset($_POST['attfield_1']) && $_POST['attfield_1'] == 'colorstyle') 
		{
			...
			$jointable_prdsnp = 'data_imagepaths.product_snapshot';
			$jointable_hrez = 'data_imagepaths.post_ready_original';
...
//				}
		//////		
			...
			if(isset($_POST['checkdate']) && $_POST['datemo'] !== '0')
				...
				$stylequery = "SELECT tzimg.colorstyle, tpsnp.brand, tpsnp.production_status, tzimg.file_path, tzimg.photo_date, threz.file_path, threz.alt
					FROM $qtable tzimg
					JOIN $jointable_hrez threz ON tzimg.colorstyle = threz.colorstyle
					LEFT JOIN $jointable_prdsnp tpsnp ON tzimg.colorstyle = tpsnp.colorstyle
					WHERE tzimg.photo_date = '$photodate'
					ORDER by tzimg.photo_date DESC, tzimg.colorstyle ASC, threz.alt ASC";
	...
	////////////////// DATE SEARCH DONT WORK YET SO ELSE IT IS		
			...
			{
			$stylequery = "SELECT threz2.colorstyle, tpsnp3.brand, tpsnp3.production_status, tpsnp3.vendor_style, threz2.photo_date, threz2.file_path, threz2.alt
					FROM $jointable_hrez threz2
					
					LEFT JOIN $jointable_prdsnp tpsnp3 ON threz2.colorstyle = tpsnp3.colorstyle
					WHERE threz2.colorstyle like '$colorstyle%'
					ORDER by threz2.photo_date DESC, threz2.colorstyle ASC, threz2.alt ASC";
			}
...
elseif (isset ($stylequery))
    ...
    while($row = mysql_fetch_array($result))
        ...
        print '<table border="1" width="auto" cols="2">
					...
					<tr>
					  <a href="' . $row[5] . '"><img src="' . $row[5] . '" width="100" height="120" alt="' . $row[0] . '" /></a>
