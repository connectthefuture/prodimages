...
def subproc_magick_large_jpg(img, destdir=None):
    ...
    if not regex_alt.findall(img):
        ...
        if regex_valid_style.findall(img):
            ...
            # '-crop',
            # str(
            # subprocess.check_output(['convert', img, '-virtual-pixel', 'edge', '-blur', '0x15', '-fuzz', '1%', '-trim', '-format', '%wx%h%O', 'info:-'], stdin=None, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False))
            # ,
            # CENTERING & Trim
            '-background',
            'white',
