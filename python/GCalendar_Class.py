...
class GCalendar:
    ...
    def __init__(self, email, password):
        ...
        self.cal_client = gdata.calendar.client.CalendarClient(source='Default')
    ...
    def _InsertSimpleWebContentEvent(self):
        ...
        type = 'image/gif'
        web_content_link = gdata.calendar.data.WebContentLink(title=title, href=href, link_type=type, web_content=web_content)
    ...
    def _InsertWebContentGadgetEvent(self):
        ...
        # Create a WebContent object
        url = 'https://raw.github.com/relic7/prodimages/master/html_css_js/html/compiledPhotoReport.html'
        web_content = gdata.calendar.data.WebContent(url=url, width='300', height='136')
        web_content.web_content_gadget_pref.append(gdata.calendar.data.WebContentGadgetPref(name='color', value='green'))

        # Create a WebContentLink object that contains the WebContent object
        title = 'Date and Time Gadget'
        #href = 'http://l.yimg.com/iu/api/res/1.2/kmmjlHIcZ2lt1S31W1XRUw--/YXBwaWQ9eXZpZGVvO2NoPTg2MDtjcj0xO2N3PTY1NztkeD0yO2R5PTE7Zmk9dWxjcm9wO2g9ODU7cT0xMDA7dz02NQ--/http://l.yimg.com/j/assets/i/us/sp/v/mlb/players_l/20130405/7054.1.jpg'
    ...
    def _AddExtendedProperty(self, event, name='http://www.example.com/schemas/2005#mycal.id', value='1234'):
    ...
    def _InsertRecurringEvent(self, title='Weekly Tennis with Beth', content='Meet for a quick lesson', where='On the courts', recurrence_data=None):
        ...
        new_event = self._InsertEvent(title, content, where, recurrence_data=recurrence_data, start_time=None, end_time=None)
    ...
    def _InsertCalendar(self, title='titleid', description='descfull', time_zone='America/New_York', hidden=False, location='locv', color='#2952A3'):
    ...
    def _UpdateCalendar(self, calendar, title='New Title', color=None):
        ...
        print 'Updating the calendar titled "%s" with the title "%s"' % (calendar.title.text, title)
        calendar.title = atom.data.Title(text=title)
    ...
    def _InsertSubscription(self, id='python.gcal.test%40gmail.com'):
    ...
    def _UpdateCalendarSubscription(self, id='python.gcal.test%40gmail.com', color=None, hidden=None, selected=None):

        """Updates the subscription to the calendar with the specified ID."""

        print 'Updating the calendar subscription with ID: %s' % id
        calendar_url = ('http://www.google.com/calendar/feeds/default/allcalendars/full/%s' % id)
    ...
    def _DeleteCalendarSubscription(self, id='python.gcal.test%40gmail.com'):
    ...
    def _batchRequest(self, updateEntry, deleteEntry):
        ...
        start_time = time.strftime('%Y-%m-%dT%H:%M:%S.000Z', time.gmtime())
        end_time = time.strftime('%Y-%m-%dT%H:%M:%S.000Z', time.gmtime(time.time() + 3600))
        insertEntry.when.append(gdata.calendar.data.When(start=start_time, end=end_time))
    ...
    def Run(self, delete='false'):
        ...
        singevent = self._InsertSingleEvent()
        singevent_up_title = self._UpdateTitle(singevent, 'New title for single event')
        singevent_up_reminder = self._AddReminder(singevent_up_title, minutes=30)
        singevent_up_ext_prop = self._AddExtendedProperty(singevent_up_reminder, name='propname', value='propvalue')
        recurevent = self._InsertRecurringEvent()
        ...
        (quick_add_event, singevent_up_ext_prop) = self._batchRequest(singevent_up_ext_prop, quick_add_event)

        # Delete entries if delete argument='true'
        if delete == 'true':
            print 'Deleting created events'
            self.cal_client.Delete(singevent_up_ext_prop)
            self.cal_client.Delete(recurevent)
...
def main():
  ...
  # parse command line options
    try:
        ...
        print ('python GCalendar_Class.py --user [username] --pw [password] ' + '--delete [true|false] ')
        sys.exit(2)
    ...
    if user == '' or pw == '':
        print ('python calendarExample.py --user [username] --pw [password] ' + '--delete [true|false] ')
        sys.exit(2)
