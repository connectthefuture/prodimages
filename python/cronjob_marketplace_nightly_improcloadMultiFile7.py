...
def sqlQuery_GetMarketplaceIncompletePOs():
    import sqlalchemy,sys
    orcl_engine = sqlalchemy.create_engine('oracle+cx_oracle://prod_team_ro:9thfl00r@borac101-vip.l3.bluefly.com:1521/bfyprd11')
    updateonly_flag = 'not'
    update_time = 20
    connection = orcl_engine.connect()
    query_marketplace_ALL_ponumbers = "SELECT DISTINCT POMGR.PO_LINE.PO_HDR_ID as po_number FROM POMGR.SUPPLIER_INGEST_STYLE RIGHT JOIN POMGR.SUPPLIER_INGEST_SKU ON POMGR.SUPPLIER_INGEST_SKU.STYLE_ID = POMGR.SUPPLIER_INGEST_STYLE.ID LEFT JOIN POMGR.SUPPLIER_INGEST_IMAGE ON POMGR.SUPPLIER_INGEST_STYLE.ID = POMGR.SUPPLIER_INGEST_IMAGE.STYLE_ID RIGHT JOIN POMGR.PO_LINE ON POMGR.PO_LINE.PRODUCT_COLOR_ID = POMGR.SUPPLIER_INGEST_STYLE.BLUEFLY_PRODUCT_COLOR RIGHT JOIN POMGR.PRODUCT_COLOR ON POMGR.PRODUCT_COLOR.ID = POMGR.PO_LINE.PRODUCT_COLOR_ID WHERE (POMGR.PRODUCT_COLOR.IMAGE_READY_DT IS {1} NULL and POMGR.SUPPLIER_INGEST_IMAGE.URL IS not NULL and POMGR.SUPPLIER_INGEST_IMAGE.IMAGE_NUMBER <= 6) and (POMGR.SUPPLIER_INGEST_IMAGE.CREATED_DATE > trunc(sysdate-{10}) and POMGR.PRODUCT_COLOR.VENDOR_STYLE NOT LIKE '%{0}%' ORDER BY 1 desc".format('VOID', updateonly_flag, str(update_time))

    #query_marketplace_postyles = "SELECT DISTINCT POMGR.SUPPLIER_INGEST_STYLE.BLUEFLY_PRODUCT_COLOR as colorstyle, POMGR.PO_LINE.PO_HDR_ID as po_number, POMGR.SUPPLIER_INGEST_STYLE.VENDOR_ID as vendor_name, POMGR.SUPPLIER_INGEST_STYLE.VENDOR_BRAND as vendor_brand, POMGR.SUPPLIER_INGEST_STYLE.VENDOR_STYLE as vendor_style, POMGR.SUPPLIER_INGEST_STYLE.BLUEFLY_CATEGORY as product_folder, POMGR.SUPPLIER_INGEST_IMAGE.URL as image_url, POMGR.SUPPLIER_INGEST_IMAGE.DOWNLOADED as download_status, POMGR.SUPPLIER_INGEST_IMAGE.IMAGE_NUMBER as alt, POMGR.SUPPLIER_INGEST_IMAGE.STYLE_ID as genstyleid, POMGR.PRODUCT_COLOR.COPY_READY_DT as copy_ready_dt, POMGR.PRODUCT_COLOR.IMAGE_READY_DT as image_ready_dt, POMGR.PRODUCT_COLOR.PRODUCTION_COMPLETE_DT as production_complete_dt, POMGR.PRODUCT_COLOR.ACTIVE as active, POMGR.SUPPLIER_INGEST_SKU.THIRD_SUPPLIER_ID as third_supplierid, POMGR.SUPPLIER_INGEST_STYLE.CREATED_DATE as ingest_dt FROM POMGR.SUPPLIER_INGEST_STYLE RIGHT JOIN POMGR.SUPPLIER_INGEST_SKU ON POMGR.SUPPLIER_INGEST_SKU.STYLE_ID = POMGR.SUPPLIER_INGEST_STYLE.ID LEFT JOIN POMGR.SUPPLIER_INGEST_IMAGE ON POMGR.SUPPLIER_INGEST_STYLE.ID = POMGR.SUPPLIER_INGEST_IMAGE.STYLE_ID RIGHT JOIN POMGR.PO_LINE ON POMGR.PO_LINE.PRODUCT_COLOR_ID = POMGR.SUPPLIER_INGEST_STYLE.BLUEFLY_PRODUCT_COLOR RIGHT JOIN POMGR.PRODUCT_COLOR ON POMGR.PRODUCT_COLOR.ID = POMGR.PO_LINE.PRODUCT_COLOR_ID WHERE POMGR.PO_LINE.PO_HDR_ID LIKE '%{0}%' and (POMGR.PRODUCT_COLOR.IMAGE_READY_DT IS {1} NULL and POMGR.SUPPLIER_INGEST_IMAGE.URL IS not NULL) and (POMGR.SUPPLIER_INGEST_IMAGE.IMAGE_NUMBER <= 6 AND POMGR.PRODUCT_COLOR.VENDOR_STYLE NOT LIKE '%VOID%') ORDER BY POMGR.SUPPLIER_INGEST_STYLE.BLUEFLY_PRODUCT_COLOR Nulls Last, POMGR.SUPPLIER_INGEST_STYLE.CREATED_DATE DESC Nulls Last, POMGR.SUPPLIER_INGEST_STYLE.VENDOR_ID Nulls Last".format(po_number, updateonly_flag)
    query_result = connection.execute(query_marketplace_ALL_ponumbers)
    results = []
    for row in query_result:
        results.append(row['po_number'])
    connection.close()
    return results


def sqlQuery_GetIMarketplaceImgs(po_number=None, all_flag=None, updateonly_flag='', update_time=None):
    import sqlalchemy,sys
    orcl_engine = sqlalchemy.create_engine('oracle+cx_oracle://prod_team_ro:9thfl00r@borac101-vip.l3.bluefly.com:1521/bfyprd11')
    if updateonly_flag:
        updateonly_flag = 'not'
    ...
    # or POMGR.SUPPLIER_INGEST_IMAGE.MODIFIED_DATE > trunc(sysdate-{0}))
    # prod complete null vs image null as above only on po search
    query_result = connection.execute(query_marketplace_postyles)
    results = {}
    for row in query_result:
        styledata = {}
...
############################################################ RUN ##################################################
############################################################ RUN ##################################################
def download_marketplace_urls(image_url):
    import requests, re, os, httplib2
...
def get_marketplace_urls(styles_dict):
    countimage = 0
    countstyle = 0
    update_styles = []
...
def main():
    ...
    countstyle = 0
    update_styles = []
## Setup Over Begin Functions
#1)
    po_numbers = sqlQuery_GetMarketplaceIncompletePOs()
#2)
    styles_dict = {}
    for po_number in po_numbers:
        styles = sqlQuery_GetIMarketplaceImgs(po_number=po_number)
...
if __name__ == '__main__':
    ...
    imagedir = os.path.abspath('/mnt/Post_Complete/Complete_Archive/MARKETPLACE/SWI')
