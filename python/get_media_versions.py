...
def query_version_number(colorstyle):
    ...
    if type(colorstyle) == list and len(colorstyle) > 1:
        ...
        q= """SELECT DISTINCT
              ...
              POMGR.PRODUCT_COLOR.VERSION                 AS VERSION0,
              POMGR.PRODUCT_COLOR_DETAIL.MEDIA_VERSION    AS "media_version",
              POMGR.PRODUCT_COLOR.MODIFIED_DATE           AS MODIFIED_DATE1
            FROM
              POMGR.PRODUCT_COLOR
            ...
            WHERE
              POMGR.PRODUCT_COLOR_DETAIL.PRODUCT_COLOR_ID in {0}
            AND POMGR.PRODUCT_COLOR.VENDOR_STYLE NOT LIKE '%VOID%'
            ORDER BY
              image_ready_dt DESC Nulls Last""".format(colorstyles)
    else:
        ...
        q= """SELECT DISTINCT
            ...
            AND POMGR.PRODUCT_COLOR.VENDOR_STYLE NOT LIKE '%VOID%'
            ORDER BY
              image_ready_dt DESC Nulls Last""".format(colorstyle)
    #else:
