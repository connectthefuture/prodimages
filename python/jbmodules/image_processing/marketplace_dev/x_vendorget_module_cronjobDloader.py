...
def update_q(**kwargs):
    ...
    if(kwargs.get('vendor', '')) > 1 and kwargs.get('all'):
        update_query.replace("AND pomgr.supplier_ingest_image.created_date > image_ready_dt + 3", "")
...
def sqlQuery_GetIMarketplaceImgs(**kwargs):
    ...
    if kwargs.get('update') and not kwargs.get('style_number'):
        query_marketplace_inprog = update_q(vendor=vendor, date_range=kwargs.get('date_range', '5'), all=kwargs.get('all'))
