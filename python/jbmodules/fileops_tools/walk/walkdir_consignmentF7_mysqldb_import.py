...
def walkeddir_parse_stylestrings_out(walkeddir_list):
    ...
    regex_consigANY = re.compile(r'^/.+?/consignment/.+?/[0-9]{9}?_?[1-9]?_?[0-9]{,4}?.*?\.[pnjpgJPGCR2]{3}$')
    regex_consigConvertedPNG = re.compile(r'^/.+?/consignment/.+?/[0-9]{9}_?.*?\.[png]{3}$')
...
#                                print "IOError on {0}".format(line)
                ...
                #file_path = file_path.replace('/mnt/Post_Ready/zProd_Server/imageServer7/var/consignment/', '/consignment/')
                ...
                #file_path_reletive = file_path.replace('/mnt/Post_Ready/zProd_Server/imageServer7/var/consignment/', '/consignment/')
...
try:
    ...
    rootdir = '/mnt/Post_Ready/zProd_Server/imageServer7/var/consignment'
...
regex = re.compile(r'^/.+?/consignment/.+?/[0-9]{9}_?.*?\.[png]{3}$')
regex_consigANY = re.compile(r'^/.+?/consignment/.+?/[0-9]{9}?_?[1-9]?_?[0-9]{,4}?.+?\.[pnjpgJPGCR2]{3}$')
...
#            connection.execute("""INSERT INTO production_consignment (colorstyle, photo_date, file_path, alt, shot_number) VALUES (%s, %s, %s, %s, %s)""", v['colorstyle'], v['photo_date'], v['file_path'],  v['alt'], v['shot_number'])
