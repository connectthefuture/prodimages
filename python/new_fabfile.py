...
def djdam():
    ...
    env.repository = 'https://bitbucket.org/relic7/djdam_hg'
    #  hosts to deploy your project, users must be sudoers
    env.hosts = ['johnb@192.168.20.59', ]
    ...
    env.django_user = 'johnb'
    # user group
    env.django_user_group = env.django_user
    #  the code of your project will be located here
    env.django_user_home = join('/home', env.django_user)
    #  projects path
    env.projects_path = join(env.django_user_home, 'virtualenvs')
    ...
    env.django_media_url = '/var/www/srv/media/'
    env.django_media_root = env.code_root
    #  django static url and root dir
    env.django_static_url = '/var/www/srv/static/'
    env.django_static_root = env.code_root
    #  do you use south in your django project?
    env.south_used = True
    #  virtualenv root
    env.virtenv = join(env.django_user_home, 'virtualenvs', str(env.project).upper())
    ...
    ## env.nginx_server_name = 'prodimages.bluefly.corp'  # Only domain name, without 'www' or 'http://'
    env.nginx_server_name = 'djdam.org'  # Only domain name, without 'www' or 'http://'    
