...
def modify_dt_img_list():
    ...
    sqlQuery_vendor_img_modify_dt = "select colorstyle from image_update where modify_dt BETWEEN SYSDATE( ) - INTERVAL 5 DAY AND SYSDATE( ) + INTERVAL 2 DAY and DATE_FORMAT(create_dt,'%Y-%M-%d') > DATE_FORMAT(modify_dt,'%Y-%M-%d') ORDER BY colorstyle DESC".format()
