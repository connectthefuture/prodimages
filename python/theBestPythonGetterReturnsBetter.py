...
def main():
    ...
    daily_incomplete_query='''SELECT t1.`file_path`, t2.* FROM `www_django`.`push_photoselects` t1 join `www_django`.`product_snapshot_live` t2 on t1.`colorstyle` = t2.`colorstyle` WHERE t2.`image_ready_dt` is null ORDER BY t1.`file_path` ASC'''
