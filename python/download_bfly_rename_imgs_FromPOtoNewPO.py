...
def sqlQuery_oldimage_newpo_duplimerge(oldpo, newpo):
    ...
    query_image_duplimerge = """WITH data AS ( SELECT POMGR.PO_LINE.PO_HDR_ID AS ponumber, POMGR.PRODUCT_COLOR.ID  AS colorstyle, POMGR.PRODUCT_COLOR.VENDOR_STYLE AS vendor_style,
                                                sum( case when POMGR.PRODUCT_COLOR.IMAGE_READY_DT is not null   then 1   else null end ) is_ready
                                                FROM POMGR.PO_LINE JOIN POMGR.PRODUCT_COLOR ON POMGR.PRODUCT_COLOR.ID = POMGR.PO_LINE.PRODUCT_COLOR_ID
                                                WHERE POMGR.PO_LINE.PO_HDR_ID IN ('{0}', '{1}')
                                                GROUP BY POMGR.PO_LINE.PO_HDR_ID, POMGR.PRODUCT_COLOR.ID, POMGR.PRODUCT_COLOR.VENDOR_STYLE
                                                ORDER BY 3, 4 asc nulls last, 2 DESC
                                                )
                                                SELECT COUNT(DISTINCT data.colorstyle), data.vendor_style, MIN(data.colorstyle) as oldstyle, MAX(data.colorstyle)  as newstyle,  data.is_ready
                                                FROM data
                                                GROUP BY data.vendor_style, data.is_ready, data.colorstyle
                                                HAVING COUNT(DISTINCT data.colorstyle) = 2 AND MIN(data.ponumber) <> MAX(data.ponumber)
                                                order by data.is_ready nulls last""".format(oldpo, newpo)
